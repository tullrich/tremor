...
bool MD3Reader::readMesh(MD3Mesh* mesh) {
	...
	if (header->ofs_eof != file_in.tellg()) {
		printf("\nERROR: end of MD3 position does not match OFS_END from file\n");
		return false;
	}
...
bool MD3Reader::_readSurface(int ofs_surface, MD3Surface* surface) {
	...
	xyznormals = new md3_vertex[surf_header->num_verts];
	...
	if (pos_end != file_in.tellg()) {
		printf("\nERROR: end of surface position does not match OFS_END from file\n");
		return false;
	}
